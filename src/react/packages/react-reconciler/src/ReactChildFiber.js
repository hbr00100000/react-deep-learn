...
function createChildReconciler(shouldTrackSideEffects) {
  ...
  function placeChild(newFiber, lastPlacedIndex, newIndex) {
    ...
    // shouldTrackSideEffects是否跟踪副作用的开关
    // key、type相同但是属性变化了，这个shouldTrackSideEffects === true
    if (!shouldTrackSideEffects) {
      // During hydration, the useId algorithm needs to know which fibers are
      // part of a list of children (arrays, iterators).
      newFiber.flags |= Forked; // |=按位或
  ...
  function reconcileChildrenArray(
    ...
    // 保留第一个创建的新Fiber节点，这样就相当于保留了从当前新fiber节点到之后的一条链
    let resultingFirstChild = null;
    // 储存新生成的Fiber节点，之后用于一些条件的判断
    let previousNewFiber = null;
    let oldFiber = currentFirstChild;
    let lastPlacedIndex = 0; // 每次循环中最后复用的Fiber的Index
    let newIdx = 0;
    ...
    // 2. oldFiber遍历完成之后，newChildren没有遍历完成
    if (oldFiber === null) {
      // If we don't have any more existing children we can choose a fast path
      // since the rest will all be insertions.
      for (; newIdx < newChildren.length; newIdx++) {
        // 1. mount: 根据传入的newChildren依次创建newFiber
        // createChild的作用
        // 1. 创建newFiber
        // 2. newFiber绑定returnFiber
        const newFiber = createChild(returnFiber, newChildren[newIdx], lanes);
        if (newFiber === null) {
          continue;
        }
        // 创建之后要一个插入的操作吧，这个就是了，给Fiber的index赋值，并计算lastPlacedIndex的值给下一个Fiber算位置
        lastPlacedIndex = placeChild(newFiber, lastPlacedIndex, newIdx);
        if (previousNewFiber === null) {
          // TODO: Move out of the loop. This only happens for the first run.
          // 之前没有newFiber那么这个就是记录第一个Fiber了
          resultingFirstChild = newFiber;
        } else {
          // 因为能进入当前逻辑那就不是第一个Fiber了，并且由于进入的函数是用于处理数组元素的
          // 所以给当前元素绑定在上一个的sibling上，他们就是兄弟啦
          previousNewFiber.sibling = newFiber;
        }
        // 记录当前Fiber
        previousNewFiber = newFiber;
      ...
      // 走到这里就相当于newChildren遍历完了，将第一个Fiber返回，也就返回了一颗小小的Fiber树
      return resultingFirstChild;
    }

    // Add all children to a key map for quick lookups.
    // 将当前显示的oldFiber以及oldFiber所有的兄弟节点加入一个map
