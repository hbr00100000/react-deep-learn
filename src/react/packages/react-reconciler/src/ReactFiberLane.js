...
export function getNextLanes(root, wipLanes) {
  /*
  1. 从跟节点上面获取待处理更新的位掩码集合'pendingLanes';
  2. 如果没有待处理更新，返回NoLanes表示没有需要处理的更新；
  3. 定义nextLanes表示下一个要更新的优先级；
  4. 获取需要立即执行的位掩码集合’nonIdlePendingLanes‘；
  5. 依据'nonIdlePendingLanes'判断nextLanes；
  6. 如果nextLanes为空，说明当前没有需要处理的更新；
  7. 如果当前有正在处理的更新并且不等同于下一个要处理的更新，
  分别取出当前正在处理的更新集合和下一个要处理的更新集合中的最高优先级的更新；
  8. 如果下一个更新的优先级>=当前正在处理的更新优先级 or  下一个最高优先级的更新 === DefaultLane && 正在处理的最高优先级更新是过度优先级 返回 当前正在处理的更新集合；
  9. 否则返回下一个更新集合；
  */
  ...
  // 获取pendingLanes中的NonIdleLanes，标识需要立即执行的位掩码集合
  const nonIdlePendingLanes = pendingLanes & NonIdleLanes;
  if (nonIdlePendingLanes !== NoLanes) {
    // 从非空闲更新集合中去除掉暂停的更新集合 得到 需要立即执行又不是被挂起的更新集合
    const nonIdleUnblockedLanes = nonIdlePendingLanes & ~suspendedLanes;
    if (nonIdleUnblockedLanes !== NoLanes) {
      // 获取需要立即执行的更新中优先级最高的更新
  ...
  if (
    ...
    // &: 这个符号其实可以理解成交集的意思，下面的含义就是wipLanes 与 suspendedLanes没有交集在·
...
export function markStarvedLanesAsExpired(root, currentTime) {
  ...
  while (lanes > 0) {
    /* 
      下面两个步骤是将最高lane从lanes中剥离出来
      案例
      lanes： 0011；
      pickArbitraryLaneIndex： 31 - clz32()；
      index: 1;
      lane: 0001 << 1;
      lane: 0010;
    */
    ...
    if (expirationTime === NoTimestamp) {
      ...
      if (
        ...
        // 如果suspendedLanes不包括lane或者pengedLanes包括lane就符合条件
